/**
 * zipread.js — Lector de ZIP mínimo, para importar una biblioteca entera de
 * golpe (un .zip de archivos ChordPro, o un .onsongarchive exportado de OnSong).
 *
...
async function inflar(bytes) {
  if (typeof DecompressionStream === 'undefined') {
    throw new Error('Este navegador no puede descomprimir el archivo; extrae el .zip y sube los archivos sueltos.');
  }
...
/** Extensiones que tiene sentido intentar importar. */
export const EXTENSIONES = ['cho', 'chopro', 'chordpro', 'crd', 'pro', 'chord', 'onsong', 'txt', 'text'];

export function pareceCancion(nombre = '') {
  const limpio = nombre.split('/').pop() || '';
